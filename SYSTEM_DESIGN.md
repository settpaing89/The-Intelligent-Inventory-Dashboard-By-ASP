...
> **Current stage:** Stage 9 complete (filter/metric enhancements: year + VIN filters, dual-handle days-in-inventory slider, `DashboardStats`) → next up is a dedicated visual styling pass (spec in `DESIGN.md` at the repo root) applying design tokens across the whole app, deliberately kept separate from this stage's functional-only wiring; after that, verification in a real browser and video recording.
...
- [ ] Visual styling pass (spec: `DESIGN.md`)
- [ ] Video recording
